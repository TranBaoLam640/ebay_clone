...
import type {
  OrderDetail,
  OrderItem,
} from "@/features/checkout/services/checkout-api";
...
export function BuyerInrRequestModal({
  ...
  const eligibility = useMemo(() => {
    if (!item)
      return { eligible: false, message: "Choose an order item first." };
    if (!order.shipment)
      return {
        eligible: false,
        message: "Shipment details are not available yet.",
      };
    const eta = new Date(order.shipment.estimatedDeliveryAt);
    if (Number.isNaN(eta.getTime()))
      return {
        eligible: false,
        message: "Estimated delivery date is unavailable.",
      };
    ...
    if (Date.now() > windowEnd) {
      return {
        eligible: false,
        message: "The item-not-received reporting window has ended.",
      };
    }
    return {
      eligible: true,
      message: "You can send a request to the seller for this item.",
    };
  }, [item, order.shipment]);
  ...
  const lineAmount =
    item.itemSubtotal ??
    (item.unitPrice != null ? item.unitPrice * item.quantity : 0);
  const requestAmount = Math.round(
    (lineAmount * quantityMissing) / Math.max(item.quantity, 1),
  );
        ...
        <div className="flex gap-3 rounded-lg border border-border bg-surface-2 p-3">
          ...
          <div className="min-w-0 flex-1">
            ...
            {order.shipment && (
              <p className="mt-1 text-xs text-muted">
                Estimated delivery:{" "}
                {formatDateTime(order.shipment.estimatedDeliveryAt)}
              </p>
            )}
          </div>
        </div>
        ...
        {existingRequest ? (
          <div className="rounded-lg border border-primary/30 bg-primary/5 p-4">
            ...
            <p className="mt-1 text-sm text-muted">
              You can review the status or message the seller from the request
              detail page.
            </p>
          </div>
        ) : step === "info" ? (
          <div className="space-y-3">
            <div className="rounded-lg border border-border bg-surface p-4">
              <p className="font-semibold text-text">
                We hope you get your item soon
              </p>
              <p className="mt-1 text-sm text-muted">
                Send this request to the seller so they can help with the
                missing item. SBay records the request and keeps the seller
          ...
          <div className="space-y-4">
            <fieldset className="space-y-2">
              <legend className="text-sm font-semibold text-text">
                Tell the seller what you prefer
              </legend>
              <RadioOption
                label="I still want the item"
                checked={resolution === "WANT_ITEM"}
                onChange={() => setResolution("WANT_ITEM")}
              />
              <RadioOption
