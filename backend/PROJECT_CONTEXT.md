...
User4: `RETURN_WINDOW_DAYS` is a positive integer (default `30`); `PAYPAL_SIMULATION_ENABLED` is `true|false` (default `true`) and gates the simulation provider actions. The complete authoritative inventory is `.env.example` and `src/config/env.js`.
